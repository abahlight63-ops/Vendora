...
// Vendora AI suggestions, size shapes the dashboard checklist, channels
...
const VOLUMES = [ // Q4: chats per day? (drives the honest plan hint on Billing!)
  ...
  { v: '10-50', t: '10 – 50', sub: 'Healthy flow — right in Free\'s sweet spot, Pro waiting.', cheer: 'Healthy flow — you are exactly who Vendora was built for.' },
...
export default function Welcome() {
  ...
  async function save() { // FINAL save: one write for all five answers (step 4 → dashboard!)
    if (!finalNiche) { setStep(0); return pop('err', 'Pick one first', 'Tell us what you sell so Vendora speaks your hustle.'); } // safety net (Q1 is the ONE required answer!)
  ...
  function next() { // advance (Q1 gates — the rest skip freely!)
    if (step === 0 && !finalNiche) return pop('err', 'Pick one first', 'Tell us what you sell so Vendora speaks your hustle.');
        ...
        <div key={step} className="quiz-step"> {/* key={step} = remount per step → entrance animation replays EVERY time! */}
          ...
          {step === 2 && ( // Q3 — channels, multi-pick (drives Connect highlights!)
            <>
              ...
              {channels.length > 0 ? <p className="quiz-cheer quiz-pop"><Ic n="checkCircle" s={15} /> {channels.length === 1 ? 'One channel — we will make it shine.' : `${channels.length} channels — Vendora covers every one.`}</p> : null}
            </>
          )}
