...
function fromAddress() {
  return process.env.EMAIL_FROM || 'Vendora <onboarding@resend.dev>'; // Resend free sandbox default
}
  ...
  return `
    <div style="font-family:Segoe UI,Arial,sans-serif;max-width:520px;margin:auto;background:#f6faf8;border-radius:14px;overflow:hidden;">
      <div style="background:#075E54;padding:20px 24px;text-align:center;">
        <img src="${base}/logo.png" alt="Vendora" width="44" style="border-radius:10px;background:#fff;padding:3px;" />
        <div style="color:#ffffff;font-weight:800;letter-spacing:2px;font-size:15px;margin-top:8px;">VENDORA</div>
      ...
      <div style="background:#eef4f1;padding:14px 24px;text-align:center;color:#777;font-size:.78rem;line-height:1.7;">
        ...
        <a href="${base}/login" style="color:#075E54;">Open Vendora</a> · <a href="${base}/help" style="color:#075E54;">Help center</a>
      </div>
    </div>`;
}
...
async function sendVerificationEmail(email, token) {
  ...
  return sendEmail({
    to: email,
    subject: 'Confirm your email — Vendora',
    html: layout({
...
async function sendOTPEmail(email, code) {
  return sendEmail({
    to: email,
    subject: `${code} — your Vendora code`,
    html: layout({
      title: 'Your Vendora code',
...
async function sendWelcomeEmail(email, name) {
  ...
  return sendEmail({
    to: email,
    subject: 'Welcome to Vendora — your shop never sleeps',
    html: layout({
      title: `Welcome, ${first}!`,
...
async function sendInactiveEmail(email, name) {
  ...
  return sendEmail({
    ...
    html: layout({
      ...
      intro: `It's been a while since you opened Vendora, ${first}. Your catalog and settings are exactly where you left them — pick up in seconds:`,
